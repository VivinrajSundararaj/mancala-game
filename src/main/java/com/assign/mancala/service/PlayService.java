...
public class PlayService {
	 ...
	 * @return @{@link MancalaBoard} after the move
	 */
	public MancalaBoard doMove(Game game, Player player, int position) {
	 ...
	 * @param position to move
	 * @return @{@link MancalaBoard} after move
	 */
	public MancalaBoard handleFirstPlayerMove(Game game, int position) {
		// Get GameBoard
		MancalaBoard board = boardService.getBoardByGame(game);
	 ...
	 * @return @{@link MancalaBoard} after move
	 */
	public MancalaBoard handleSecondPlayerMove(Game game, int position) {
		// Get GameBoard
		MancalaBoard board = boardService.getBoardByGame(game);
	 ...
	 */
	public void checkCapture(MancalaBoard board, int index, int lower, int upper, int store) {
		// Check if between boundaries, i.e. landed on his own house and if the house
		// was empty
		if (index >= lower && index <= upper && pitService.getPitNumberOfStonesByBoardAndPosition(board, index) == 1) {
	 ...
	 *
	 * @param game  @{@link Game} to check for
	 * @param board @{@link MancalaBoard} to check for
	 * @return boolean if the game is finished
	 */
	public boolean checkFinished(Game game, MancalaBoard board) {
